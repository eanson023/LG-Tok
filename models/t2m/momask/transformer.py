...
class MaskTransformer(nn.Module):
    ...
    def trans_forward(self, motion_ids, cond, padding_mask, force_mask=False):
        ...
        output = self.seqTransEncoder(xseq, src_key_padding_mask=None)[1:] #(seqlen, b, e)
    ...
    def forward_with_cond_scale(self,
        ...
        if force_mask:
            ...
            return self.trans_forward(motion_ids, cond_vector, None, force_mask=True)

        # logits = self.trans_forward(motion_ids, cond_vector, padding_mask)
        logits = self.trans_forward(motion_ids, cond_vector, None)
        if cond_scale == 1:
            return logits

        # aux_logits = self.trans_forward(motion_ids, cond_vector, padding_mask, force_mask=True)
        aux_logits = self.trans_forward(motion_ids, cond_vector, None, force_mask=True)
    ...
    def generate(self,
        ...
        device = next(self.parameters()).device
        seq_len = num_latent_token
        batch_size = len(conds)
        m_lens = torch.full((batch_size,), fill_value=seq_len).to(device)
        ...
        ids = torch.full((batch_size, seq_len), fill_value=self.mask_id).to(device)
        scores = torch.zeros(batch_size, seq_len).float().to(device)
        ...
        ids = torch.where(ids==self.mask_id, -1, ids)
...
class ResidualTransformer(nn.Module):
    def __init__(self, code_dim, cond_mode, latent_dim=256, ff_size=1024, num_layers=8, cond_drop_prob=0.1,
        ...
        if shared_codebook:
            token_embed = torch.normal(mean=0, std=0.02, size=(_num_tokens, code_dim))
            # 先expand再clone，确保内存独立
            self.token_embed_weight = nn.Parameter(token_embed.expand(opt.num_quantizers-1, _num_tokens, code_dim).clone())
            if share_weight:
                self.output_proj_weight = self.token_embed_weight
                self.output_proj_bias = None
            else:
                output_proj = torch.normal(mean=0, std=0.02, size=(_num_tokens, code_dim))
                output_bias = torch.zeros(size=(_num_tokens,))
                self.output_proj_weight = nn.Parameter(output_proj.expand(opt.num_quantizers-1, _num_tokens, code_dim).clone())
                self.output_proj_bias = nn.Parameter(output_bias.expand(opt.num_quantizers-1, _num_tokens).clone())
    ...
    def trans_forward(self, motion_codes, qids, cond, padding_mask, force_mask=False):
        ...
        output = self.seqTransEncoder(xseq, src_key_padding_mask=None)[2:]  # (seqlen, b, e)
    ...
    def forward_with_cond_scale(self,
        ...
        if force_mask:
            ...
            logits = self.trans_forward(motion_codes, qids, cond_vector, None, force_mask=True)
            logits = self.output_project(logits, qids-1)
            return logits

        # logits = self.trans_forward(motion_codes, qids, cond_vector, padding_mask)
        logits = self.trans_forward(motion_codes, qids, cond_vector, None)
        logits = self.output_project(logits, qids-1)
        if cond_scale == 1:
            return logits

        # aux_logits = self.trans_forward(motion_codes, qids, cond_vector, padding_mask, force_mask=True)
        aux_logits = self.trans_forward(motion_codes, qids, cond_vector, None, force_mask=True)
    ...
    def forward(self, all_indices, y, m_lens):
        ...
        logits = self.trans_forward(history_sum, active_q_layers, cond_vector, None, force_mask)
    ...
    def generate(self,
        ...
        motion_ids = torch.where(motion_ids==-1, self.pad_id, motion_ids)
        ...
        for i in range(1, num_quant_layers):
            ...
            logits = self.forward_with_cond_scale(history_sum, i, cond_vector, None, cond_scale=cond_scale)
