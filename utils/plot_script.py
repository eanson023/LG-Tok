...
# def plot_3d_motion_static_frames(save_path, kinematic_tree, joints, title='', figsize=(10, 10), radius=4, num_samples=8):
#     matplotlib.use('Agg')
...
#     # Normalize trajectory origin
#     data[..., 0] -= data[:, 0:1, 0]
#     data[..., 2] -= data[:, 0:1, 2]
#     trajec -= trajec[0:1]

#     # Sample 8 frames
#     frame_ids = np.linspace(0, len(data) - 1, num_samples, dtype=int)
...
#     # Plot XZ plane
#     minx, maxx = MINS[0] - trajec[:, 0].max(), MAXS[0] - trajec[:, 0].min()
#     minz, maxz = MINS[2] - trajec[:, 1].max(), MAXS[2] - trajec[:, 1].min()
#     verts = [
#         [minx, 0, minz],
#         [minx, 0, maxz],
#         [maxx, 0, maxz],
...
#     # Plot trajectory
#     ax.plot3D(trajec[:, 0], np.zeros_like(trajec[:, 0]), trajec[:, 1], linewidth=1.5, color='blue', alpha=0.6)

#     # Plot skeletons at key frames
#     colors = ['red', 'blue', 'black', 'red', 'blue',
#               'darkblue', 'darkblue', 'darkblue', 'darkblue', 'darkblue',
#               'darkred', 'darkred', 'darkred', 'darkred', 'darkred']
#     for idx in frame_ids:
#         offset_x, offset_z = trajec[idx, 0], trajec[idx, 1]
#         for i, (chain, color) in enumerate(zip(kinematic_tree, colors)):
#             linewidth = 4.0 if i < 5 else 2.0
#             x = data[idx, chain, 0] - offset_x
#             y = data[idx, chain, 1]
#             z = data[idx, chain, 2] - offset_z
#             ax.plot3D(x, y, z, linewidth=linewidth, color=color, alpha=0.9)
...
def plot_3d_motion_static_frames(save_path, kinematic_tree, joints, title='', figsize=(10, 10), radius=4, num_samples=4):
    matplotlib.use('Agg')
    ...
    center = (MINS + MAXS) / 2
    ax.set_xlim3d([center[0] - radius / 2, center[0] + radius / 2])
    ax.set_ylim3d([0, radius])
    ax.set_zlim3d([center[2] - radius / 2, center[2] + radius / 2])
...
if __name__ == "__main__":
    # joints = np.load('dataset/HumanML3D/new_joints/000004.npy')
    import os
    import natsort
    import random
    from tqdm import tqdm
    data_dir = 'dataset/HumanML3D/new_joints'
    save_dir = 'dataset/HumanML3D/motion_img_plt'
    paths = []
    ...
    for p in tqdm(paths, total=len(file_list)):
        joints = np.load(p)
        name = p.split('/')[-1]
        if joints.ndim == 2 or len(joints) <1:
            continue
        ...
        if os.path.exists(save_path):
            continue
        plot_3d_motion_static_frames(save_path, t2m_kinematic_chain, joints)